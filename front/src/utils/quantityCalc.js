 ...
 *
 * Phase 1: 하드코딩 테이블에서 조회.
 * Phase 2: 이 함수 본문만 Agent/RAG API 호출로 교체 — 반환 타입 동일.
 *
 * @param {string}            elementType   예: 'IfcColumn'
 * @param {number}            totalVolumeM3 해당 그룹 체적 합계 (m³)
 * @param {'KDS'|'AIJ'|'ACI'} standard
 * @returns {{
 *   rebar:    {value, unit, formula, reason} | null,
 *   formwork: {value, unit, formula, reason} | null,
 *   concrete: {value, unit, formula, reason},
 *   curing:   {value, unit, formula, reason} | null,
 * }}
 */
export function resolveQuantity(elementType, totalVolumeM3, standard = 'KDS') {
  ...
  return {
    rebar: density
      ? { value: +(vol * density.avg).toFixed(1), unit: 'kg', formula: density.formula, reason: density.reason }
      : null,
    formwork: fw
      ? { value: +(vol * fw.ratio).toFixed(2), unit: 'm²', formula: fw.formula, reason: fw.reason }
      : null,
    concrete: {
      value: vol,
      unit: 'm³',
      formula: `${elementType} 부재 체적 합계`,
      reason: '콘크리트 타설량 = 부재 체적(바운딩박스 × 보정계수)',
    },
    curing: cur
      ? { value: cur.days, unit: '일', formula: cur.formula, reason: cur.reason }
      : null,
  };
}
