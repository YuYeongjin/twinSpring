...
const FORMULA_HELP = {
  windSpeed: {
    ...
    sub: 'h_factor = ((z+H)/10 + 1)^0.25  (height amplification)',
  },
  seismic: {
    title: 'Seismic Force',
...
function runAnalysis(modelData, env, loads, matId) {
  ...
  return modelData.map((el, idx) => {
    ...
    const sZ = Math.max(Number(el.sizeZ) || 1, 0.01); // 높이(Height)
    const pZ = Number(el.positionZ) || 0;
    const type = el.elementType || 'IfcColumn';

    const selfWt = sX * sY * sZ * mat.density;
    ...
    if (type === 'IfcColumn' || type === 'IfcPier') {
      ...
      const hFactor = Math.pow((pZ + sZ) / 10 + 1, 0.25);
      const Fwind = qDesign * hFactor * sX * L;
      const Fseismic = seismic.Sa * axialLoad;
      const Flateral = Math.max(Fwind, Fseismic);

      bendingMoment = Flateral * L / 2;
      shearForce = Flateral;
      ...
      axialLoad = selfWt + loads.deadLoad * sX * sY * loads.numFloors;
      const Fw = qDesign * sX * sZ;
      const Fs = seismic.Sa * axialLoad;
      shearForce = Math.max(Fw, Fs);
      bendingMoment = 0;

    } else if (type === 'IfcSlab') {
      const span = Math.min(sX, sY);
      A = 1.0 * sZ;
      I = (sZ ** 3) / 12;
      c = sZ / 2;
      L = Math.max(span, 0.1);

      const q = loads.deadLoad + loads.liveLoad + env.snowLoad + sZ * mat.density;
      bendingMoment = q * (L ** 2) / 10;
      shearForce = q * L / 2;
      axialLoad = 0;
    ...
    const eps = 1e-9;
    const sigmaAxial = axialLoad / (Math.max(A, eps) * 1000);
    const sigmaBending = (bendingMoment * c) / (Math.max(I, eps) * 1000);
    const sigmaShear = 1.5 * shearForce / (Math.max(A, eps) * 1000);
