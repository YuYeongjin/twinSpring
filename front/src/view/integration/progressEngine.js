...
export const EQUIP_LABEL = { excavator: 'Excavator', dump: 'Dump Truck', crane: 'Crane' };

export const TASK_RULES = {

  IfcSlab: {
    blockers: [
      { type: 'excavator', min: 1, reason: 'No excavator → foundation excavation impossible' },
      { type: 'dump',      min: 1, reason: 'No dump truck → soil removal impossible' },
    ],
    equipBonus: [
      { type: 'excavator', perUnit: 0.40 },
      { type: 'dump',      perUnit: 0.25 },
    ],
    workerBonus:      0.12,
    minWorkers:       1,
    fewWorkerPenalty: 0.60,
  },

  IfcColumn: {
    blockers: [
      { type: 'crane', min: 1, reason: 'No crane → column lifting impossible' },
    ],
    equipBonus: [
  ...
  IfcBeam: {
    blockers: [
      { type: 'crane', min: 1, reason: 'No crane → beam lifting impossible' },
    ],
    equipBonus: [
  ...
  IfcPier: {
    blockers: [
      { type: 'crane',     min: 1, reason: 'No crane → pier concrete pour/lifting impossible' },
      { type: 'excavator', min: 1, reason: 'No excavator → pier foundation excavation impossible' },
    ],
    equipBonus: [
      { type: 'crane',     perUnit: 0.50 },
...
export function calcProgressRate(elementType, workers, equipment) {
  ...
  if (!rule) return { rate: 0, blocked: true, reason: `${elementType}: no task rule defined` };
...
export function getRecommendations(elementType, workers, equipment) {
  ...
  for (const req of rule.blockers) {
    ...
    if (needed > 0) {
      recs.push({
        priority: 'critical',
        text: `Add ${needed} ${EQUIP_LABEL[req.type]} → work can proceed`,
      });
    }
  }
  ...
  if (workerCount < (rule.minWorkers || 0)) {
    ...
    recs.push({
      priority: 'warning',
      text: `Need ${needed} more workers (current: ${workerCount} → min: ${rule.minWorkers})`,
    });
  }
  ...
  if (!isBlocked) {
    for (const bonus of rule.equipBonus) {
      ...
      recs.push({
        priority: 'boost',
        text: `Add 1 ${EQUIP_LABEL[bonus.type]} → speed +${pct}%`,
      });
    }
